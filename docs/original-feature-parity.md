...
6. **No entitlements / no Info.plist usage strings.** *(partly resolved)* `NSLocationWhenInUseUsageDescription` was added in **S1.2** and `NSBluetoothAlwaysUsageDescription` in **S3.1** (honest wording: BLE recon for security auditing, local-only, no auto-connect). Still missing: `*.entitlements`, Automation/Contacts strings, `com.apple.security.network.client` (the latter only matters under the App Sandbox = S8.x). Location/Bluetooth now surface a real auth prompt + remediation instead of failing closed silently.
