...
| WEP weak scheduling (FMS/KoreK) | `AirCrackWrapper.m`, `WaveNetWEPWeakCrack.m` | Engine intact but **unreachable** (tautology guard `ScanControllerScriptable.m:718`) | CPU-only | n/a | 🔧 `modernized` (fix reachability + `keyLen104` copy-paste bug) |
...
**Permissions:** outbound network (Kismet) → `com.apple.security.network.client`; file read/write via panels; crash reporter used Contacts (`ABAddressBook`).

**Test steps:** import a known pcap → network/client counts match a golden value (`capture-validation-agent`); round-trip `.kismac` save/open with no data loss (`report-export-test-agent`); each export opens in its target app; Kismet connect against a modern Kismet server in a lab (`capture-validation-agent`). Confirm server-export and crash-upload paths are gone, not silently failing.
